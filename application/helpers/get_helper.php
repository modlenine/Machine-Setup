...
{
    if (!empty($_FILES)) {
        $targetDir = "upload/images_temp/";
        $formno = $_POST['file_main_formno'];
        $filetype = $_POST['file_type'];
        $user = getUser()->Fname . " " . getUser()->Lname;
        $ecode = getUser()->ecode;
        ...
        $filenameNotResize = substr_replace($fileName,  $formno . "-" . $uniqueFileName . "-" . $fileTypeCode.".". $filename_type, 0);
        $filenameResize = substr_replace($fileName,  $formno . "-" . $uniqueFileName . "-" . $fileTypeCode, 0);
        ...
        if($path_parts['extension'] == "jpeg" || $path_parts['extension'] == "jpg"){
            $newWidth = 1000;
            resize($newWidth, "upload/images_temp/" . $filenameResize , $_FILES["file"]["tmp_name"]);
        }else{
            move_uploaded_file($_FILES["file"]["tmp_name"], "upload/images_temp/" . $filenameNotResize);
        }
        
        // ตรวจสอบว่าไฟล์ถูกอัพโหลดหรือไม่
        // if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFilePath)) {
        //     $arsaveFileData = array(
        //         "file_detail_formno" => $detailFormno,
        //         "file_main_formno" => $formno,
        //         "file_name" => $fileName
        //     );
        //     echo json_encode(["status" => "success", "fileName" => $fileName, "message" => "ไฟล์ถูกอัพโหลดสำเร็จ!"]);
        // } else {
        //     echo json_encode(["status" => "error", "message" => "เกิดข้อผิดพลาดในการอัพโหลดไฟล์!"]);
        // }
        $arsaveFileData = array(
            "file_main_formno" => $formno,
            "file_name" => $filenameNotResize,
            "file_type" => $filetype,
            "file_userpost" => $user,
            "file_ecodepost" => $ecode,
            "file_deptcodepost" => $deptcode,
            "file_datetime" => Date("Y-m-d H:i:s")
        );
        if(gfn()->db->insert("msd_files_temp" , $arsaveFileData))
        {
            echo json_encode(["status" => "success", "fileName" => $filenameNotResize, "message" => "ไฟล์ถูกอัพโหลดสำเร็จ!"]);
        }else{
            echo json_encode(["status" => "error", "message" => "เกิดข้อผิดพลาดในการอัพโหลดไฟล์!"]);
        }
...
function uploadImage_edit()
{
    if (!empty($_FILES)) {
        $targetDir = "upload/images_temp/";
        $mainformno = $_POST['file_main_formno'];
        $detailformno = $_POST['file_detail_formno'];
        $filetype = $_POST['file_type'];
        $user = getUser()->Fname . " " . getUser()->Lname;
        $ecode = getUser()->ecode;
        $deptcode = getUser()->DeptCode;
        ...
        $filenameNotResize = substr_replace($fileName,  $mainformno . "-" . $uniqueFileName . "-" . $fileTypeCode.".". $filename_type, 0);
        $filenameResize = substr_replace($fileName,  $mainformno . "-" . $uniqueFileName . "-" . $fileTypeCode, 0);
...
function del_fileupload()
{
    ...
    if (!empty($data->fileName)) {
        ...
        gfn()->db->where("file_name" , $data->fileName);
        gfn()->db->delete("msd_files_temp");
